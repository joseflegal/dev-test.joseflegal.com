...
export default {
  ...
  signup({ name, email, password, organisation_id }) {
    return http.post("accounts/signup", { name, email, password, organisation_id }).then(res => res.data);
  },
  ...
  updatePassword({ password, passwordToken }) {
    return http.post(`accounts/new_password/${passwordToken}`, { password }).then(res => res.data);
  },
